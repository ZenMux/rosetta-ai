    ...
    return {
      ...
      usage: {
        ...
        output_tokens_details: {
          reasoning_tokens: (usage as any)?.output_tokens_details?.thinking_tokens ?? 0,
        },
      },
      ...
      case "content_block_delta": {
        ...
        if (delta.type === "thinking_delta") {
          ...
          if (state.currentBlockType === "server_tool_use_web_search") {
            const lastEntry =
              state.webSearchByToolUseId.size > 0
                ? Array.from(state.webSearchByToolUseId.values())[
                    state.webSearchByToolUseId.size - 1
                  ]
                : null;
            if (lastEntry) {
              lastEntry.query += delta.partial_json;
            }
            ...
            events.push({
              ...
              output_index: lastToolCall?.outputIndex ?? 0,
